...
def generate_data():
    ...
    costs = np.round(np.random.uniform(cost_range[0], cost_range[1], size=total_products), 2)
    benefit_factors = np.round(np.random.uniform(benefit_factor_range[0], benefit_factor_range[1], size=total_products), 2)    
    benefits = np.round(costs * benefit_factors, 2)
    
    products = {}
    for i, product_name in enumerate(product_names):
        products[product_name] = {"cost": costs[i], "benefit": benefits[i]}
    ...
    # revenue
    revenue_list = np.array([products[product]["benefit"] for product in product_name_list])
