...
class StoreProductDashboard(Board):
    """
    ...
    def __init__(self, self_board: Board):
        """
        Initializes the StoreProductDashboard object.
        ...
        self.order = 0
        self.order_tabs = 0
        self.menu_path = "Store Product Overview"
    ...
    def plot(self):
        ...
        self.plot_sales_percentage_by_store(product=1, temporality="")
        self.plot_sales_accumulated_by_store(product=1, temporality="Current Date")
    ...
    def plot_header(self) -> bool:
        ...
        """
        title_html = create_title_name_head(
            title="Store & Product Performance", subtitle=""
        )
    ...
    def plot_kpi_indicators(self) -> bool:
        ...
        columns = ["Stores", "Total Sales", "Average Sales", "Products"]
    ...
    def plot_tabs(self, temporality: str) -> bool:
        ...
        self.plot_sales_percentage_by_store(temporality, product=0)
        self.plot_sales_accumulated_by_store(temporality, product=0)
    ...
    def plot_sales_by_store(self, temporality: str) -> bool:
        ...
        sales_by_store = self.df_app["Sales Products by Store"][temporality]
        self.shimoku.plt.free_echarts(
            raw_options=format_raw_options(sales_by_store),
            order=self.order,
            rows_size=4,
            cols_size=12,
            title="Sales Products by Store",
        )
        self.order += 1
        return True

    def plot_sales_percentage_by_store(
        self, temporality: (str, None), product: int = 0
    ) -> bool:
        """
        ...
        Args:
            temporality (str): The selected temporality (e.g., "Current Week").
            product (int): Indicates whether to plot sales by product (1) or by store (0).

        Returns:
            bool: True if the operation is successful.
        """
        if product == 1:
            data = self.df_app["Sales Product Percentage by Store"]
            ...
            data = self.df_app["Sales Percentage by Store"][temporality]
            self.shimoku.plt.pie(
                data=data,
                order=self.order,
                names="store_id",
    ...
    def plot_sales_accumulated_by_store(
        self, temporality: str, product: int = 0
    ) -> bool:
        """
        ...
        if product == 1:
            data = self.df_app["Sales Accumulated by Product"]
            title = "Sales by Product (Accumulated)"
        else:
            data = self.df_app["Sales Accumulated by Store"][temporality]
